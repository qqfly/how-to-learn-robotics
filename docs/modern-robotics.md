...
有些小伙伴第一次接触李群李代数时会说：「这些方法似乎只是涉及一个表征问题，似乎并没有什么太大的优势。」这个认识是**不对且危险**的。就像我们刚开始学线性代数的时候，看到矩阵乘法就说「这不就是把多元一次方程组换个写法嘛」——一开始就拒绝新的思考方式，等后面线性组合、零空间、映射这些真正有用的工具出现的时候就会比较吃力，甚至可能因为一开始就拒绝，以至于没有机会接触更高级的工具。

李群李代数就是描述 SO(3)、SE(3) 这类空间的经典数学工具。学完这一章，你之前对于四元数、角速度之类的疑问将一扫而空。
...
（要计算矩阵的 $\alpha$ 次幂，就需要引入矩阵指数与矩阵对数，它们都是从群加法推导衍生出来的。）
...
下面进入本章的重头戏：用两个实际问题，看看这套语言到底能做什么。它们有个共同特点——用传统方法要么做不了，要么做得很别扭；而在李群的语言下，解法几乎是「显然」的。
...
而在李群的语言下，答案水到渠成。回想一下 Bezier 曲线是怎么构造的：$n$ 阶 Bezier 曲线，就是用 $n$ 层线性插值嵌套出来的。我们已经有了任意李群空间的直线插值定义，自然也就得到了任意李群空间的 Bezier 曲线（多项式曲线）：
...
这时候，你可能已经尝试搭建过一些机器人平台，了解了一些基本的控制理论。但是，你发现实际的机器人并不理想：加工装配有偏差，动力学模型可能非常不精确。要让前面的理论落到真机上，还差几步。

**运动学标定。** 机器人的实际几何参数与图纸不一致，直接用名义参数计算运动学，末端精度就上不去。标定的基本套路，是构造一个误差方程，再用数值优化求解。以「让机械臂末端以不同姿态触碰同一个固定点」为例：以固定点为原点（随意）建立坐标系 $\{P\}$，则末端点位置满足 ${}^{P}\mathbf{p} = f(\mathbf{q}, \boldsymbol{\phi})$，其中 $\boldsymbol{\phi}$ 是全部 DH 参数。由于末端始终与固定点接触，误差方程为：

$$\Delta\mathbf{p} = \mathbf{0} - {}^{P}\mathbf{p} = J\,\Delta\boldsymbol{\phi},\qquad J = \begin{bmatrix} \dfrac{\partial f}{\partial \phi_1} & \dfrac{\partial f}{\partial \phi_2} & \cdots \end{bmatrix}$$

测量几组数据，用高斯牛顿之类的方法迭代更新 $\boldsymbol{\phi}^{k+1} = \boldsymbol{\phi}^{k} + (J^{\top}J)^{-1}J^{\top}\Delta\mathbf{p}$，直到收敛，就得到了机器人当前的运动学参数。这里的关键操作是「对参数求雅可比」——你会发现李群李代数可以非常方便地定义各种相关的雅可比。更系统的内容可以看 Khalil 的教材《Modeling, identification and control of robots》<sup>[4]</sup>，其中你还需要了解各种滤波算法（计算加速度）、各种数值优化算法。

**动力学辨识。** 多轴机器人的动力学方程虽然复杂，但可以整理成对参数**线性**的形式：

$$\boldsymbol{\tau} = A(\mathbf{q}, \dot{\mathbf{q}}, \ddot{\mathbf{q}})\,\boldsymbol{\Theta}$$

其中 $\boldsymbol{\Theta}$ 是一组待辨识的参数组合（base parameters set）。你看，这就是熟悉的 $Ax=b$，用最小二乘就能求解。实施的时候有几个细节：激励轨迹不能随便选（系统辨识理论里有专门的方法）；角速度、角加速度没法直接测量，需要滤波处理（而且输入输出要用**同一个**滤波器，避免引入不一致的延时）；摩擦力如果采用线性模型，可以放进上式一起辨识。
...
有了准确的动力学模型，就可以直接用驱动器反馈的**电流**估计外力矩——传说中的 Sensor-less Collision Detection。灵敏到什么程度呢：一张纸碰上去，机械臂就能停下来。检测到碰撞后，再切换成「重力补偿 + 小增益 PD」，就是拖动示教（Hand Guiding）。后来，我又在 Baxter 上把整臂的版本做了一遍。从做控制的角度看，机器人本就不存在「关节位置控制」这个概念：电源 → 驱动器 → 电机 → 减速器 → 实际环境，这条链路上的所有部分都是通过能量连接在一起的，任何一段的能量输入都会引起其他部分的变化。完整的实验过程可以看[《听说现在协作机器人很火，所以我也做了 1/7 个》](https://mp.weixin.qq.com/s/hkZjZItqyfwG6k0cwRm9kA)与[《我把剩下的 6/7 协作机器人给做完了》](https://mp.weixin.qq.com/s/1qyXJ01n0mcMyRX6ZB6jOQ)。

<img src="images/collision-detection.webp" width="500" alt="协作机器人碰撞检测 / 力控演示"/>

**摩擦力，入坑需谨慎。** 上面的实验是直驱平台，可以不考虑摩擦力。换一个极端例子：100kg 负载、86 减速比的行星齿轮传动，不考虑摩擦力时，动力学模型完全无法拟合力矩曲线；而即使想把摩擦力辨识出来，尝试几种摩擦力经验公式，也没办法完全消除误差——到目前为止，我们还没有一个通用、权威的摩擦力模型。

<img src="images/friction-fit-fail.webp" width="500" alt="大减速比传动下，不考虑摩擦力的模型完全无法拟合实测力矩"/>

这就是为什么绝大多数协作机器人都要加**关节力矩传感器**：直接在减速器输出端测量力矩，跳过传动系统，绕开摩擦力。具体的测量方案也各有取舍：应变片式（如 Kinova）、双编码器式（如 UR）、串联弹性驱动器 SEA（如 Baxter），再加上无传感器的电流估计方案，精度、成本、复杂度各不相同。

现在，你能把单独的一个机器人控制好了。但你发现，机器人一旦跟环境发生接触，只用机器人模型就不够了。你需要对环境进行建模。但是，环境是无法精确建模的。于是，你开始学各种**力控**、**阻抗控制**之类的内容。相应地，你就可以亲手实现协作机器人的那些标志性功能了——碰撞触停、拖动示教、柔顺装配。
