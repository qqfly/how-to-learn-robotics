...
上一节的结尾，你发现了基于动力学前馈的 PID 控制算法。但是，前馈的效果，完全取决于你的模型准不准——运动学参数、转动惯量、质心位置……这些数字从哪里来？

从图纸上来？很遗憾，机器人加工、装配总有偏差，实际参数与图纸并不一致。让模型贴近真机，就是**标定**（Calibration）与**辨识**（Identification）要做的事。这两件事在教材里往往一笔带过，但在实际工程中几乎躲不开，所以这里单独说一说。好消息是，它们的套路高度一致：**构造一个误差方程，然后用最小二乘之类的方法求解**。

**运动学标定**。以经典的「多点触碰标定」为例：让机械臂末端以不同的姿态反复触碰同一个固定点，以固定点为原点建立坐标系 $\{P\}$，则末端位置满足 ${}^{P}\mathbf{p} = f(\mathbf{q}, \boldsymbol{\phi})$，其中 $\boldsymbol{\phi}$ 是全部 DH 参数。由于末端始终与固定点接触，就有了误差方程：

$$\Delta\mathbf{p} = \mathbf{0} - {}^{P}\mathbf{p} = J\,\Delta\boldsymbol{\phi}$$

其中 $J$ 是 $f$ 对参数 $\boldsymbol{\phi}$ 的雅可比。测量几组数据，用高斯牛顿之类的方法迭代求解，就得到了机器人当前的运动学参数。这套「误差方程 + 数值优化」的套路，在机器人标定问题里到处都是——后面进阶部分的手眼标定，也是同一个味道。用「robot kinematics calibration」之类的关键词可以搜到大量文献。

**动力学辨识**。多轴机器人的动力学方程虽然复杂，但可以整理成对参数**线性**的形式：

$$\boldsymbol{\tau} = A(\mathbf{q}, \dot{\mathbf{q}}, \ddot{\mathbf{q}})\,\boldsymbol{\Theta}$$

其中 $\boldsymbol{\Theta}$ 是一组待辨识的参数组合（base parameters set）。你看，这就是熟悉的 $Ax=b$，最小二乘就能求解。实施时有几个细节：激励轨迹不能随便选（系统辨识理论里有专门的方法）；角速度、角加速度没法直接测量，需要滤波处理，而且输入输出要用**同一个**滤波器，避免引入不一致的延时；摩擦力如果采用线性模型，可以放进上式一起辨识。系统的介绍可以看 Khalil 的教材《Modeling, identification and control of robots》<sup>[4]</sup>。
...
这就是为什么绝大多数协作机器人都要加**关节力矩传感器**：直接在减速器输出端测量力矩，跳过传动系统，绕开摩擦力。具体的测量方案各有取舍：应变片式（如 Kinova）、双编码器式（如 UR）、串联弹性驱动器 SEA（如 Baxter）。

最后：如果你按照 3.5 节的建议搭过单轴伺服平台，那么本节的所有实验——辨识、碰撞检测、拖动示教——都可以在它上面复现一遍，强烈建议试试。而机器人一旦与环境发生持续接触（打磨、装配），只靠碰撞检测就不够了，你需要去了解**力控**、**阻抗控制**这些关键词，同样推荐 Khalil 的教材<sup>[4]</sup>。
