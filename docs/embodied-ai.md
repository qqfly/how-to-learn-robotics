...
!!! note "本部分整理中"
    ...
    这一部分讨论数据驱动的机器人技术。计划中的章节：导言（整体趋势、当前状态、为什么它可能是未来方向）；机器学习快速介绍（FNN、CNN、Transformer、LLM，为后面铺垫）；强化学习与全身控制（已被验证的成功领域）；模仿学习（理论不算完备、但在限定场景有落地可能）；VLA 与世界模型等数据驱动的通用操作探索；冷静的展望；上手。下面两节暂从原「进阶」部分原文迁来，后续将随本部分一起重构。
